...
import useSports from "@/components/hooks/useGetSports"

type Props = {}

const SportPage = (props: Props) => {
  const homePageMetaData = useMetadata()
  const getHomeMeta = homePageMetaData?.find((m: any) => m.page === "/sports")
  ...
  return (
    <>
      <Helmet>
        ...
        <link rel="canonical" href={`https://slot-ndkk.vercel.app/sports`} />
      </Helmet>
        ...
        <section>
          <nav className=" font-bold text-black" aria-label="Breadcrumb">
            <ol className="inline-flex list-none truncate p-0 text-xs md:text-base ">
              ...
              <li className="flex items-center">
                <Link className="text-red-500" href="/sport">
                  SPORT
                </Link>
              </li>
            </ol>
