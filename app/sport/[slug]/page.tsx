...
import {
  useGetRelatedSportByCategory,
  useGetSportPostWithId,
} from "@/components/hooks/useGetSportPostWithId"
...
const SportsDetailsPage = () => {
  ...
  const newsDetails = useGetSportPostWithId(slug as string)
  const category = newsDetails?.category?._id
  // @ts-ignore
  const relatedPosts = useGetRelatedSportByCategory(slug as string, category)
  ...
  return (
    <>
      <Helmet>
        ...
        <link
          rel="canonical"
          href={`https://slot-ndkk.vercel.app/sport/${newsDetails?.title}`}
        />
      </Helmet>
      <section className=" mx-auto max-w-7xl px-8 pb-24">
        ...
        <article className="grid grid-cols-1 gap-4 md:grid-cols-4">
          <section className="col-span-1 flex flex-col gap-4 md:col-span-3 ">
